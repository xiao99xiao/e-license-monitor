...
- 📱 **即座の通知**: ntfy.sh を通じて中国語で通知を送信
- ⚙️ **設定可能なセレクター**: 監視要素とナビゲーションリンクをカスタマイズ
- 🔄 **自動更新**: 60秒ごとにページデータを自動更新
...
2. トピックを購読: `reserve_alert_xiao`
3. アプリがバックグラウンドで実行されていることを確認

**注意**: デフォルトの通知チャンネルは `reserve_alert_xiao` です。チャンネル名を変更するには、`content.js` の ntfy URL を修正してください：
```javascript
...
#### 空き要素セレクター
教習予約空き状況を監視する要素を指定（段階（第1段階、第2段階、検定）によって異なる値を設定する必要があります）：
...
#### 予約リンクセレクター
教習予約ページへのナビゲーションリンクを指定（段階（第1段階、第2段階、検定）によって異なる値を設定する必要があります）：
...
**Q: 通知が届かない**
- ntfy.sh アプリが実行中か確認
- 正しいチャンネルを購読しているか確認: `reserve_alert_xiao`
- ネットワーク接続を確認
- デバッグログの送信状況を確認
...
- **通知サービス**: ntfy.sh
- **デフォルト通知チャンネル**: `reserve_alert_xiao`
- **監視間隔**: 60秒
...
2. `content.js` ファイルの2箇所で ntfy URL を修正:
