...
在日本驾校预约课程非常难，通常你只能预约到很多天之后的课。这让想要段时间快速从驾校毕业变得很困难。
...
1. **在手机上安装 ntfy.sh 应用** 以获得即时通知
2. **在 `content.js` 中自定义频道主题**（默认：`e-license-reserve-alert`）
3. **仅适用于** 使用 e-license.jp 系统的驾校
