...
2. **⚠️ 重要：必须修改频道名称！**
   - 默认频道 `e-license-reserve-alert` 是共享的
   - 不修改会收到其他驾校学生的通知消息
   - 在 `content.js` 中改为你的专属频道名称
...
**⚠️ 重要警告**：默认通知频道 `e-license-reserve-alert` 是共享频道！**必须修改为你的专属频道名称**，否则会收到其他驾校学生的通知消息。

```javascript
xhr.open("POST", "https://ntfy.sh/你的专属频道名称", true);
```
